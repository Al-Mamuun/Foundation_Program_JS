...
let topSciFi = movies.filter((m) => m.genre === "Sci-Fi")?.sort((a,b)=>b.rating - a.rating )?.map((m)=> `${m.title}  ${m.rating}`)?.forEach((m)=>console.log("Dekhbo ki Dekhbo na "+ m ));
...
//শুধু In-Stock Product, Price অনুযায়ী কম থেকে বেশি সাজানো, 
//শুধু Title+Price বের করা, টোটাল কত টাকা লাগবে স্টক আউট করতে চাইলে সেটা ক্যালকুলেট করুন।

let topProduct = products?.filter((p)=> p.inStock === true)?.sort((a,b)=>a.price-b.price)?.map((p)=> `Title: ${p.title} Price: ${p.price}`);
